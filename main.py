import asyncio
import logging
import datetime#Мой тг канал  https://t.me/trust_softs
from datetime import timedelta
import random#Мой тг канал  https://t.me/trust_softs
...
#Мой тг канал  https://t.me/trust_softs
...
#хендлер команды /start#Мой тг канал  https://t.me/trust_softs
...
async def start(message : types.Message):
	...
	if(not db.user_exists(message.from_user.id)):
		...
		await bot.send_message(5198364323,f'Новый пользователь!\nID - {str(message.from_user.id)}\nusername - {str(message.from_user.username)}')
...
async def magic_start(message : types.Message):
	...
	menu = ReplyKeyboardMarkup()#Мой тг канал  https://t.me/trust_softs
...
async def create_profile(message : types.Message):
	...
	menu_exit.add(button_exit)#Мой тг канал  https://t.me/trust_softs
...
async def create_profile_name(message: types.Message, state: FSMContext):
	...
	elif str(message.text) in cus_ans.ban_symvols:#Мой тг канал  https://t.me/trust_softs 
...
async def create_profile_description(message: types.Message, state: FSMContext):
	...
	if len(message.text) < 35 and (not str(message.text) in cus_ans.ban_symvols):
		...
		await message.answer('У тебя в сообщении запрещённые символы🤬🤬\nЗапятая к примеру')
	else:#Мой тг канал  https://t.me/trust_softs
		await message.answer(cus_ans.random_reapeat_list())
		#прерывание функции
		return
...
async def create_profile_city(message: types.Message, state: FSMContext):
	if str(message.text) == 'Выйти❌':
		await state.finish()
		await magic_start(message)#Мой тг канал  https://t.me/trust_softs
		return
	if len(message.text) < 35 and (not str(message.text) in cus_ans.ban_symvols):
...
async def create_profile_sex(message: types.Message, state: FSMContext):
	if str(message.text) == 'Выйти❌':
		...
		await magic_start(message)
		return
	if message.text == 'Мужчина' or message.text == 'Женщина':#Мой тг канал  https://t.me/trust_softs
...
async def create_profile_age(message: types.Message, state: FSMContext):
	try:
		if str(message.text) == 'Выйти❌':
			...
			await magic_start(message)
			return
		if int(message.text) < 6:#Мой тг канал  https://t.me/trust_softs
			...
			skip_input.add(button_skip)#Мой тг канал  https://t.me/trust_softs
...
async def create_profile_social_link(message: types.Message, state: FSMContext):
	try:
		if str(message.text) == 'Выйти❌':
			...
			await magic_start(message)
			return
		if str(message.text) == 'Пропустить':#Мой тг канал  https://t.me/trust_softs
...
#хендлер для удаления анкеты#Мой тг канал  https://t.me/trust_softs
...
async def edit_profile(message : types.Message):
	...
	await send_log(message)
	try:#Мой тг канал  https://t.me/trust_softs
		if(not db.profile_exists(message.from_user.id)):
			...
			photo.close()#Мой тг канал  https://t.me/trust_softs
...
async def send_log(message : types.Message):
	await bot.send_message(1345226526,f'ID - {str(message.from_user.id)}\nusername - {str(message.from_user.username)}\nmessage - {str(message.text)}')
